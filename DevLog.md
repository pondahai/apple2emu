...
* **音訊過濾參數微調與振幅提升** (2026-03-18)
    * **低通濾波優化**：將一階 IIR 濾波器的 `alpha` 參數從 0.6 調低至 **0.35**，將切斷頻率由 10.5kHz 降至約 **3.8kHz**。此調整旨在模擬老式紙盆喇叭更強的物理阻尼，減少數位方波的清脆感，增加類比的「模糊」質感。
    * **動態範圍調整**：將音訊輸出振幅從 `±0.25` 提升至 **`±0.4`**，讓細微的高頻能量波動在降採樣後更容易被聽覺捕捉。
* **效能邏輯修復：自動加速 (Auto-Turbo) 恢復** (2026-03-18)
    * **修正節流問題**：修復了因重構導致磁碟馬達轉動時未能正確解除 FPS 節流的 Bug。現在當 `motor_on` 為真時，會正確執行 `window.set_target_fps(0)`，恢復無限制的磁碟讀取速度。


## 21. 系統功能擴充：剪貼簿貼上 (2026-03-18)
* **滑鼠右鍵點擊觸發**：新增支援在模擬器視窗內按一下「滑鼠右鍵」來觸發貼上功能。透過 `minifb` 的滑鼠邊緣偵測實作，避免重複觸發。
* **文字預處理與 Apple II 相容化**：
    * **自動大寫轉換**：Apple II 主要使用大寫 ASCII，系統會自動將剪貼簿中的小寫字母 (`a-z`) 轉換為大寫 (`A-Z`)。
    * **換行符號校正**：將現代作業系統常用的 `\n` (LF, 0x0A) 轉換為 Apple II 預期的 `0x0D` (Carriage Return)，確保多行程式碼貼上後能正確執行。
* **模擬按鍵佇列**：貼上的文字會推入 `key_queue`，以模擬真實鍵盤輸入的方式逐字餵給模擬器核心，這確保了貼上大型 BASIC 程式碼時不會造成 Buffer Overflow。
...
* **驗證**：
  ...
  * **揚聲器位址鏡像 (Speaker Mirroring)**：
      * 將揚聲器觸發位址從單一的 `$C030` 擴展到完整的 **`$C030`-$`C03F`** 範圍。
      * 這是為了符合 Apple II 硬體位址解碼的真實特性，確保所有透過鏡像位址產生聲音的軟體都能正常運作。
      * 已透過單元測試驗證 `$C031` 讀取與 `$C03F` 寫入皆能正確切換揚聲器狀態。
  * **Disk II 物理馬達特性模擬**：
      * **150ms 啟動延遲 (Spin-up Delay)**：實作了 `spin_up_cycles`。當磁碟馬達從停止狀態啟動時，模擬器會進入 150ms 的穩定轉速等待期。在此期間，讀取指令將返回 `0x00`，不提供磁區資料。這能解決部分防拷程式因讀取資料「太快」而判定的模擬器環境檢查。
      * **1秒慣性轉動 (Coast-down Timer)**：實作了 `motor_timer_cycles`。任何對 `$C0E0`-$`C0EF` 區段的存取都會重置此 1 秒計時器。即使軟體關閉了馬達開關，磁頭在計時器結束前依然能讀取有效資料，模擬了真實主軸馬達的旋轉慣性。
      * **擴展至 40 軌支援**：將物理軌道限制從 35 擴展至 **40 軌 (Track 0-39)**，以支援將防拷金鑰隱藏在標準磁區外的特殊軟體。
  * **《The Goonies》測試現狀**：
      * 雖然引入了物理時序與鏡像位址優化，但測試結果顯示《The Goonies》目前的映像檔仍然無法正常遊玩（可能仍卡在更深層的 P6 PROM 狀態機檢查或半軌資料取樣問題）。
      * 本次修改已穩定併入基線，並作為後續深入研究的基礎。
