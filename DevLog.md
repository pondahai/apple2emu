...
* **音訊架構重構：週期面積積分法 (Area-Based Integration)** (2026-03-18)
    * **原理更新**：捨棄複雜的 PolyBLEP 與固定網格過度採樣，改為直接計算每個 44.1kHz 採樣視窗（約 23.19 CPU 週期）內喇叭處於高電位（On）的精確時間比例（Duty Cycle）。這能 100% 捕捉所有高頻翻轉，並將其正確轉化為 PCM 能量值。
    * **類比低通濾波 (Analog LPF)**：在積分後的數值上套用一階 IIR 濾波器（模擬 RC 電路），設定約 7kHz-10kHz 的切斷頻率，用以模擬真實 Apple II 喇叭的物理慣性與電感特性。
    * **實驗觀察**：雖然理論上這是最精確的還原方式，但在執行《德軍總部》時槍聲仍呈現單音化現象。這證實了問題核心可能不在數位採樣精確度，而是在於更微觀的指令級時序（Jitter）或是遊戲迴圈與音訊窗口的相位對齊問題。
* **編譯最佳化設定 (Cargo Release Profile)** (2026-03-18)
    * 在根目錄 `Cargo.toml` 加入極限最佳化參數：開啟 **LTO (Link Time Optimization)**、強制 **Codegen Units = 1**、並設定 **Panic = Abort**。這能極大化跨 crate 的程式碼精簡與執行效率，確保在 1MHz 模擬下音訊積分運算的流暢度。
