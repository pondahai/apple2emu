...
* **極限過度採樣實驗 (8x 至 32x)**：為了徹底排除混疊（Aliasing）的可能性，將音訊內部採樣率一路拉升至 **32x (1.4112 MHz)**。此頻率已正式超過 Apple II CPU 的運行時脈（1.023 MHz），理論上應能 100% 捕捉所有喇叭切換事件。
* **持續性的單音化問題**：實驗結果顯示，即便在 1.4 MHz 的極高解析度下，《德軍總部》的槍聲依然傾向於呈現「單音」而非「隨機白噪音」。這是一個極其重要的負向實驗結果，說明問題核心不在採樣頻率不足，而可能源於：
    1. **缺乏類比物理特性模擬**：真實 Apple II 喇叭具有電感與物理慣性，會對高頻方波產生非線性的平滑效果。
    2. **指令級同步問題**：即便整體時鐘對齊，如果指令內部的寫入時機（Sub-instruction timing）與音訊採樣窗口的相位關係過於固定，隨機的 Jitter 就會變成規律的週期信號，聽起來就是單音。
* **未來研究方向**：考慮引入簡易的一階低通濾波器（RC Filter 模型）來模擬真實喇叭的物理限制，或者研究更細緻的次指令級（Sub-instruction）週期偏移。
