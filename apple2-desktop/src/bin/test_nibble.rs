...
fn main() {
    ...
    if let Some(off) = data_offset {
        ...
        let rev = {
            let mut r = [0u8; 256];
            for j in 0..64 {
                r[apple2_core::nibble::NIBBLE_WRITE_TABLE[j] as usize] = j as u8;
            }
            r
        };
        ...
        for j in 0..343 {
            decoded[j] = rev[track0.raw_bytes[off + j] as usize];
        }
        ...
        for i in 0..8 {
            println!(
                "  reconstructed[{}] = {:02X} (expected: {:02X}) {}",
                i,
                pri[i],
                disk[i],
