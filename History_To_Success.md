...
### 3. CPU 非法指令與 RWTS 尋軌解鎖 (2026-03-13)
*   **SKB/SKW Dummy Reads**：DOS 3.3 在 `$0BB8` 尋軌迴圈中，極度依賴 `SKB` (Skip Byte, 如 `$04`) 的記憶體 Dummy Read 所產生的微秒級延遲。之前實作雖然週期數對了，但少了真正的記憶體讀取，導致時序失準崩潰。
*   **輔助緩衝區 (Secondary Buffer) 最終正解**：
    *   取消了針對 Boot ROM 的雙重反向輸出，改為單純的正向發送 (Forward Emission)。
    *   保留了位元互換 `swap2`，並將組合順序回歸 `b2 << 4 | b1 << 2 | b0`。
    *   取消了所有的位移 `SOff` 的補償邏輯。
*   **結局**：完美解碼出 `$0800`，後續所有磁區也能正確通過檢查碼。MASTER.DSK 終於讀取全部 18 軌並順利進入 Applesoft BASIC 的 `]` 提示符號！

---
**最終結論**：模擬器的穩定性不僅建立在 32 週期的讀取區間，更建立在 CPU 對記憶體週期讀寫的絕對忠實（如 Dummy Read 和 Page Cross Penalty）。在軟硬體的精確配合下，DOS 3.3 才能在沒有任何作弊修改的情況下成功引導。